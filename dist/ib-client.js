...
export class IBClient {
    ...
    maxAuthAttempts = 3;
    tickleInterval;
    tickleIntervalMs = 30000; // 30 seconds (well within 1/sec rate limit)
    constructor(config) {
        this.config = config;
    ...
    async getAccountInfo() {
        ...
        try {
            ...
            for (let i = 0; i < accounts.length; i++) {
                ...
                const summaryResponse = await this.client.get(`/portfolio/${account.id}/summary`);
    ...
    async getMarketData(symbol, exchange, fields, conid) {
        try {
            let contractId = conid;
            // If conid not provided, search for contract
            if (!contractId) {
                const searchResponse = await this.client.get(`/iserver/secdef/search?symbol=${symbol}`);
            ...
            if (!fields) {
                ...
                // It's a preset name
                fieldStr = FIELD_PRESETS[fields] || FIELD_PRESETS.detailed;
            }
            else {
            ...
            const response = await this.client.get(`/iserver/marketdata/snapshot?conids=${contractId}&fields=${fieldStr}`);
            // Second call to get actual data (first call initializes stream)
            const dataResponse = await this.client.get(`/iserver/marketdata/snapshot?conids=${contractId}&fields=${fieldStr}`);
            return {
                symbol: symbol,
                conid: contractId,
    ...
    async getHistoricalData(symbol, conid, period, bar, outsideRth) {
        try {
            let contractId = conid;
            // If conid not provided, search for contract
            if (!contractId) {
            ...
            if (outsideRth !== undefined)
                params.append('outsideRth', outsideRth.toString());
    ...
    async getOptionsChain(symbol, conid, includeGreeks = true) {
        try {
            let underlyingConid = conid;
            // If conid not provided, search for underlying
            if (!underlyingConid) {
                const searchResponse = await this.client.get(`/iserver/secdef/search?symbol=${symbol}&secType=STK`);
                if (!searchResponse.data || searchResponse.data.length === 0) {
                    throw new Error(`Underlying symbol ${symbol} not found`);
                }
                const contract = searchResponse.data.find((c) => c.assetClass === 'STK' || c.secType === 'STK')
            ...
            const secdefResponse = await this.client.get(`/iserver/secdef/search?symbol=${symbol}&secType=OPT`);
            ...
            secdefResponse.data.forEach((opt) => {
                if (opt.months) {
                    opt.months.split(';').forEach((m) => months.add(m));
                }
            });
            ...
            for (const month of monthsArray) {
                try {
                    const strikesResponse = await this.client.get(`/iserver/secdef/strikes?conid=${underlyingConid}&secType=OPT&month=${month}`);
    ...
    async findOptionContract(symbol, expiration, strike, right, delta) {
        try {
            ...
            if (expiration) {
                ...
                const matchedMonth = chain.expirations.find((e) => e.month.includes(exp) || exp.includes(e.month));
                if (matchedMonth) {
                    targetMonth = matchedMonth.month;
                }
            }
            const expData = chain.expirations.find((e) => e.month === targetMonth);
            if (!expData) {
            ...
            // Find closest strike
            const closestStrike = strikes.reduce((prev, curr) => Math.abs(curr - targetStrike) < Math.abs(prev - targetStrike) ? curr : prev);
            return {
                symbol: symbol,
    ...
    async getPortfolioSummary(accountId, groupBy) {
        try {
            ...
            // Get positions
            const positionsResponse = await this.client.get(`/portfolio/${targetAccount}/positions/0`);
    ...
    async placeStockOrder(orderRequest) {
        try {
            // First, get the contract ID for the symbol
            const searchResponse = await this.client.get(`/iserver/secdef/search?symbol=${orderRequest.symbol}`);
            if (!searchResponse.data || searchResponse.data.length === 0) {
            ...
            // Place the order
            const response = await this.client.post(`/iserver/account/${orderRequest.accountId}/orders`, {
                orders: [order],
            });
    ...
    async placeOptionOrder(orderRequest) {
        try {
            let conid;
            ...
            if (orderRequest.conid) {
                ...
                const stockSearchResponse = await this.client.get(`/iserver/secdef/search?symbol=${orderRequest.symbol}`);
                ...
                const stockContract = stockSearchResponse.data.find((c) => c.assetClass === 'STK' || c.secType === 'STK')
                    || stockSearchResponse.data[0];
                const underlyingConid = stockContract.conid;
                // Search for the specific option contract
                const optionSearchResponse = await this.client.get(`/iserver/secdef/search?symbol=${orderRequest.symbol}&secType=OPT`);
                ...
                if (!optionContract) {
                    throw new Error(`Option contract not found for ${orderRequest.symbol} ` +
     ...
     */
    async confirmOrder(replyId, messageIds) {
        try {
     ...
     */
    async modifyOrder(orderId, accountId, modifications) {
        try {
     ...
     */
    async searchContracts(query, secType, exchange, currency, limit = 10) {
        try {
            ...
            if (currency) {
                results = results.filter((contract) => contract.currency === currency);
            }
            // Limit results
     ...
     */
    async getTradesHistory(accountId, days = 7) {
        try {
