...
export class ToolHandlers {
    ...
    async getMarketData(input) {
        try {
            ...
            const result = await this.context.ibClient.getMarketData(input.symbol, input.exchange, input.fields, input.conid);
            return {
                content: [
                    {
                        type: "text",
    ...
    async getHistoricalData(input) {
        try {
            ...
            const result = await this.context.ibClient.getHistoricalData(input.symbol, input.conid, input.period, input.bar, input.outsideRth);
            return {
                content: [
                    {
                        type: "text",
    ...
    async getQuote(input) {
        try {
            ...
            const result = await this.context.ibClient.getMarketData(input.symbol, undefined, "basic");
    ...
    async getOptionsChain(input) {
        try {
            ...
            const result = await this.context.ibClient.getOptionsChain(input.symbol, input.conid, input.includeGreeks !== false // Default to true
            );
            return {
                content: [
                    {
    ...
    async findOptionContract(input) {
        try {
            await this.ensureGatewayReady();
            if (this.context.config.IB_HEADLESS_MODE) {
                await this.ensureAuth();
            }
            const result = await this.context.ibClient.findOptionContract(input.symbol, input.expiration, input.strike, input.right, input.delta);
            return {
                content: [
                    {
                        type: "text",
    ...
    async getPortfolioSummary(input) {
        try {
            await this.ensureGatewayReady();
            if (this.context.config.IB_HEADLESS_MODE) {
                await this.ensureAuth();
            }
            const result = await this.context.ibClient.getPortfolioSummary(input.accountId, input.groupBy);
            return {
                content: [
                    {
                        type: "text",
    ...
    async searchContracts(input) {
        try {
            ...
            const results = await this.context.ibClient.searchContracts(input.query, input.secType, input.exchange, input.currency, input.limit);
            return {
                content: [
                    {
                        type: "text",
    ...
    async getContractDetails(input) {
        try {
            // Ensure Gateway is ready
            await this.ensureGatewayReady();
            // Ensure authentication in headless mode
