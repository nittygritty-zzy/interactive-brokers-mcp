...
async function initializeGateway(ibClient) {
    if (!gatewayManager) {
...
async function cleanupAll(signal) {
    if (signal) {
...
function IBMCP({ config: userConfig }) {
    ...
    if (logConfig.IB_PASSWORD_AUTH)
        logConfig.IB_PASSWORD_AUTH = '[REDACTED]';
    if (logConfig.IB_PASSWORD)
        logConfig.IB_PASSWORD = '[REDACTED]';
...
if (isMainModule) {
    ...
    if (logEnvConfig.IB_PASSWORD_AUTH)
        logEnvConfig.IB_PASSWORD_AUTH = '[REDACTED]';
    ...
    if (logFinalConfig.IB_PASSWORD_AUTH)
        logFinalConfig.IB_PASSWORD_AUTH = '[REDACTED]';
    ...
    if (logFinalConfigAfter.IB_PASSWORD_AUTH)
        logFinalConfigAfter.IB_PASSWORD_AUTH = '[REDACTED]';
